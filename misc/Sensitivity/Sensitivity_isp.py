...
#                           [0.35],
#                           [0.4]])
isp =  np.array([[2500, 3500, 5000, 3140]])#s
CD = np.array([[2.5]])
...
less = (alt_loc[3]-alt_loc[1])/abs((1-(isp[0][3]/isp[0][1]))*100)
more = (alt_loc[1]-alt_loc[2])/abs((1-(isp[0][2]/isp[0][1]))*100)

#plt.plot(altitude, D[:,0,0], linestyle='--', label='Min. CDA for min. density')
plt.plot(altitude, D[:,0,0], linestyle='--', label='Isp = 2500')
plt.plot(altitude, D[:,0,1], linestyle='--', label='Isp = 3500')
plt.plot(altitude, D[:,0,2], linestyle='--', label='Isp = 5000')
plt.plot(altitude, D[:,0,3], linestyle='-', label='Isp = 3140')

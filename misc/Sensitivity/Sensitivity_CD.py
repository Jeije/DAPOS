...
for a in range(m):
    for n in range(k):
        if (D[n,0,a] - D_design) < 0.00000000001:
            alt_loc[a] = n
            break

less = (alt_loc[1]-alt_loc[0])/abs((1-(CD[0][0]/CD[0][1]))*100)
more = (alt_loc[3]-alt_loc[1])/abs((1-(CD[0][3]/CD[0][1]))*100)

#plt.plot(altitude, D[:,0,0], linestyle='--', label='Min. CDA for min. density')
plt.plot(altitude, D[:,0,0], linestyle='--', label='CD = 2.00')
plt.plot(altitude, D[:,0,1], linestyle='--', label='CD = 2.50')
plt.plot(altitude, D[:,0,2], linestyle='--', label='CD = 3.00')
plt.plot(altitude, D[:,0,3], linestyle='-', label='CD = 2.79')
